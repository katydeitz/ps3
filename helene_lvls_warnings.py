...
with open('/blue/bsc4452/share/Class_files/data/CO-OPS_8727520_wl.csv', 'r', encoding='utf-8') as file:
    ...
    for row in reader:
        ...
        if len(row) < 4:
            # Warning for missing data point
            print(f"Warning: No reading received at {row[0]} {row[1]} (missing data).")
            continue
        ...
        if not water_level_str:
            # Warning for missing water level data
            print(f"Warning: No valid water level reading at {date} {time}.")
            continue
        ...
        try:
            ...
            print(f"Warning: Unable to convert water level at {date} {time} to a float.")
            continue
        ...
        if previous_water_level is not None:
            ...
            # Warning if water level rises more than 0.25 ft
            if water_level_change > 0.25:
                print(f"Warning: Water level increased by {water_level_change:.2f} ft between {previous_time} and {date} {time}.")

        # Warning if water level exceeds 5.0 feet
        if water_level > 5.0:
            print(f"Warning: Water level exceeds 5.0 ft at {date} {time} (level: {water_level:.2f} ft).")
